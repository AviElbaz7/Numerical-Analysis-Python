...
class Assignment3:
    ...
    def areabetween(self, f1: callable, f2: callable) -> np.float32:
        ...
        """
        object1 = Assignment2()
        intersect = object1.intersections(f1, f2, 1, 100, 1e-9)

        area_func = lambda x: f1(x) - f2(x)
        if len(intersect) < 2:
            return np.NaN
        area = 0
        for i in range(len(intersect) - 1):
            area += abs(self.integrate(area_func, intersect[i], intersect[i + 1], 300))
